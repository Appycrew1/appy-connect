...
-- providers: public can read only active; admins full
create policy "providers read public active" on public.providers
  for select using (is_active = true);
create policy "providers admin all" on public.providers
  for all using (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));

-- listing_submissions: public can insert; admins can read/manage
create policy "submissions insert public" on public.listing_submissions
...
-- contact_messages: public can insert; admins can read/manage
